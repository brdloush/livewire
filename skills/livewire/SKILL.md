...
The generator matches property names case-insensitively against a priority table:
`firstName` → `faker.name().firstName()`, `isbn` → `faker.code().isbn13()`,
`*Year` suffix → year between 1850–2024, `*At`/`*Since` suffix → past date, etc.
String values are clamped to `@Column(length=…)` when available.
