  ...
  (try
    ...
    (binding [*ns* (the-ns 'user)]
      (eval '(require '[net.brdloush.livewire.core :as lw]
                      ...
                      '[net.brdloush.livewire.faker :as faker]
                      '[net.brdloush.livewire.callgraph :as cg])))
    (catch Exception e
...
(defn start!
  ...
  (if @server-atom
    ...
    (let [server (nrepl/start-server :port port)]
      ...
      (println (str "[livewire] nREPL server started on port " port " with user aliases (lw, q, intro, trace, qw, hq, jpa, mvc, faker, cg)")))))
